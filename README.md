...
- Snapshot bundle creation/read, snapshot-recipe validation/save, manual artifact
  override, and asynchronous generation. Generated checkpoints can feed resumed
  E2E tests or offline microtests. Generation uses the generic job get/cancel tools
...
offline microtests but cannot start a Checkpoint E2E.
...
### Checkpoint E2E Workflow

1. Find the E2E and checkpoint with `qa_catalog_search` and
   `qa_snapshot_bundle_get`, then read the YAML with `qa_definition_get`.
2. Keep `test_type: e2e` explicit and add a `snapshot` path relative to the
...
5. Poll `qa_job_get`. The `preparing` events expose checkpoint loading, isolated
   ...
   `checkpoint_restored` event hands control to normal execution.
